...
//				.put("password", request.getPassword()).put("userID", "User_" + count).put("numberOfCharacter", 0);
//
...
//					.setData(Protocol.CharacterData.newBuilder().setName(data.getString("name"))
//							.setOccupation(data.getString("occupation")).setLevel(data.getInt("level"))
//							.setStrength(data.getInt("strength")).setMagic(data.getInt("magic"))
//							.setDefense(data.getInt("defense")).setSpeed(data.getInt("speed"))
//							.setDame(data.getInt("dame")).setArmor(data.getInt("armor")))
//					.setPosition(Protocol.CharacterPosition.newBuilder().setMapID(position.getString("mapID"))
//							.setX(position.getDouble("x")).setY(position.getDouble("y")))
//					.setStatus(Protocol.CharacterStatus.newBuilder().setCurHP(status.getInt("curHP"))
//							.setMaxHP(status.getInt("maxHP")).setCurMP(status.getInt("curMP"))
...
//
//		JsonObject characterData = JsonObject.create().put("name", request.getName())
//				.put("occupation", request.getOccupation()).put("level", 1).put("strength", 1).put("magic", 1)
//				.put("defense", 1).put("speed", 1).put("dame", 1).put("armor", 1);
//		staticBucket.upsert(
//				JsonDocument.create(request.getUserID() + "_Character_" + numberOfCharacter + "_Data", characterData));
//
//		JsonObject characterPosition = JsonObject.create().put("mapID", "Map_1").put("x", 0.0).put("y", 0.0);
//		staticBucket.upsert(JsonDocument.create(request.getUserID() + "_Character_" + numberOfCharacter + "_Position",
//				characterPosition));
//
//		JsonObject characterStatus = JsonObject.create().put("maxHP", 100).put("curHP", 100).put("maxMP", 100)
//				.put("curMP", 100);
//		staticBucket.upsert(JsonDocument.create(request.getUserID() + "_Character_" + numberOfCharacter + "_Status",
//				characterStatus));
//
...
//		JsonObject characterPosition = JsonObject.create().put("mapID", request.getNewPosition().getMapID())
//				.put("x", request.getNewPosition().getX()).put("y", request.getNewPosition().getY());
